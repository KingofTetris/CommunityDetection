...
class non_overlap_game:
    ...
    def community_detection_game(self, outdirpath, fname,datasetType=1):
        ...
        partitionLA = self.Lables_to_Partition(LA) #LA是算法分区
        Q = modularity(self.G,partitionLA)
        ...
        original = {}
        i = 0
        for community in partitionLA:
            i = i + 1
            original[i] = community
        #开始尝试合作
        ...
        while cooperItern < maxit:  # 开始博弈，实验maxit次
            ...
            #要copy一下
            partition = original.copy() #每个合作博弈大回合的开始要还原成初始的分区结果
            isChange = True
            best = -1 #最佳合并社区的编号
            ...
            while isChange is True:  # 如果有改变就继续博弈
                ...
                '''
                print("当前社区数" + str(len(partition.keys())))
        ...
        while itern < maxit:  # 开始博弈，实验maxit次
            ...
            while isChange is True:  # 如果有改变就继续博弈
                ...
                # 开始比较三个动作之后效用函数的大小
                if join[1] > leave:
                    self.node_community[node] = join[
                        0]  # 加入这个邻居社区  #TODO 原来会越来越好，原来玄机在这里，后面的博弈都是在前面的基础上，前面已经让这些节点加入了大概率会更好的社区 所以后面的博弈效果都很好
