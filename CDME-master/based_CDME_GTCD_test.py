...
class non_overlap_game:
    ...
    def utility_function(self,u,c): # 节点u在社区c中的收益
       ...
       for node in self.G.nodes():
           if self.node_community[node] == c:
               ...
               #现在就是简单的亲密度求和再平均
               #后面可能要考虑一下改进效用函数
               #实际上平均都可以去掉了， 只是普通的平均对结果没什么影响。
               paysoff_u = paysoff_u + (list[0] + list[1])
               '''
               #先预留一下重叠的位置
               u_comm = self.node_community[u]
               node_comm = self.node_community[node]
               #之后重叠社区必然会让决策是一个集合，而不是简单的一个数字
               itersection = set(u_comm) & set(node_comm) 
               paysoff_u = paysoff_u + (list[0] / len(u_comm) + list[1] / len(node_comm) ) * len(itersection)
               '''
       return  paysoff_u
    ...
    #loss = 1/2m (|si| - 1)
    def utility_function3(self,u,c): # 节点u在社区c中的收益
        gain_function = 0
    ...
    def community_detection_game(self, outdirpath, fname,datasetType=1):
        ...
        while cooperItern < maxit:  # 开始博弈，实验maxit次
            ...
            max_game_number = math.comb(len(partition.keys()),2) #最大博弈次数Cn2
            game_number = 0 #初始设为0
            merging_number = 0 #真正合并的次数
            chose_community = -1 #记录合并的社区
            merging_community = -1 #记录合并的社区
            #开始合作博弈
            while isChange is True:  # 如果有改变就继续博弈
                    ...
                    if key != community: #如果当前选中的社区不一样
                        ...
                        if new_key_utility[key] > all_coaliation_utility[key] and new_com_utility[community] > all_coaliation_utility[community]:
                            ...
                            if deta_x > max_deta_x:
                               ...
                               flag = True #是真的能合并的社区，而不是上一次记录的best。
        ...
        #现在的合作博弈没有把NMI改得更好，加上这段话那合作博弈的社区就被掩盖了
        # self.node_community = max_node_community.copy()
    ...
    #其实下面应该还有个do_nothing 但其实就是等于的情况就什么都不用做
    def do_nothing(self):
        pass
    ...
    def coaliation_utility(self,partition):
        ...
        '''
        for key in partition.keys():
            s_degree_sum = self.s_degree_sum(key,partition) #每个社区的总度，把partition也传进去，避免遍历整个网络。
            s_per = 0.0 #内在度
        ...
        for key in partition.keys():
            ...
            if len(partition[key]) == 1:
                ...
                #其他情况就遍历合并社区中的节点，计算每个节点的内在度除以自己的度，然后全部加起来
                sum = 0.0
                for node in partition[key]:
                    sum = sum + self.per(node,self.node_community[node]) / self.deg[node]
