...
class non_overlap_game:
    ...
    def community_detection_game(self, outdirpath, fname,datasetType=1):
        ...
        while itern < maxit: #开始博弈，实验maxit次
            ...
            while isChange is True: #如果有改变就继续博弈
                #重新映射G的节点顺序，做张新的表node_mapping
                #这个和随机选择的区别在于 一定能让所有节点都进行选择。
                node_mapping = dict(zip(self.G.nodes(), sorted(self.G.nodes,key=lambda k: random.random())))
                G_new = nx.relabel_nodes(self.G,node_mapping)
    ...
    def coaliation_utility(self,partition):
        ...
        #     # 算完一个赋一个
        #     # Q(S)= 2e(S)/|S| - (|S|/2|E|)^2 前面是内聚性程度，后面是惩罚项，合并成一个社区时惩罚最大，但其实4E^2是个固定常数，根本没什么计算的价值。
        #     s_utility = (2 * s_per / s_degree_sum) - (s_degree_sum / (2 * self.edge_count))**2
        #     # Q(S)= e(S)/|E| - (|S|/2|E|)^2
