...
class non_overlap_game:
    ...
    '''
    修改亲密度计算公式 改为 I(i,j) = AA(i,j) * f(i,j)
    引入节点互动频率
    令f(i,j)= C/ki+kj-2C 其中C是公共邻居的数量
    从公式上就是公共邻居对他们其他度的比例，越高就是互动越频繁
    '''
    def simintimacy(self,u,v):
        ...
        #还要考虑一下如果2C等于ki+kj的情况
        #那么他们的所有度都是他们的公共邻居
        #那么他们其实是完全镜像的两个节点直接返回1
        if(self.deg[u] + self.deg[v] - 2 * len(comm_neighbors) == 0):
            return [self.deg[u],self.deg[v]]
        else:
            f_uv = len(comm_neighbors) / (self.deg[u] + self.deg[v] - 2 * len(comm_neighbors))
        init_u_to_v = AA_uv * f_uv * self.deg[u]
        init_v_to_u = AA_uv * f_uv * self.deg[v]
        #返回亲密度
    ...
    def utility_function(self,u,c): # 节点u在社区c中的收益
       ...
       for v in self.G.neighbors(u):
           ...
           if (v_community == c): #如果v在社区c中，那么才会计算收益
               ...
               paysoff_u = paysoff_u + 1/2 * (initValue[0] + initValue[1]) * (u_community & v_community)
            ...
            if deg_node == 1:  # 如果节点的度为1，那么它的社区就是它唯一的那个邻居编号
                ...
                if flag is False:  # 如果遍历完flag为false
                    ...
                    for neig in neighbors:
                        #计算亲密度
                        nodesimdeg = (self.simintimacy(node,neig)[0] + self.simintimacy(node,neig)[1]) / 2
                        #print(type(nodesimdeg))
