...
class non_overlap_game:
    ...
    # C阻尼系数取值(0,1) 一般设置为0.6-0.8
    #随机实现simRank
    #采用缓存来存储计算结果，避免重复计算，同时通过限制递归深度来减少递归次数。可以根据具体情况调整递归深度的大小。
    #但结果上单纯的simRank很耗时而且效果很差
    def simRank(self,u,v,C = 0.7,depth=3,cache=None): #默认设为0.7,C越小递归调用的深度就越少
        ...
        #因为是无向图那么入邻居就是邻居
        neigh_u = list(self.G.neighbors(u))
        neigh_v = list(self.G.neighbors(v))

        len_u = len(neigh_u)
        len_v = len(neigh_v)

        #在调用 random.choice() 之前需要确保邻居集合非空
        # ，否则会报 list index out of range 的错误。可以添加一个判断邻居集合是否为空的条件，若为空则直接返回0。
        #如果结合为空直接返回0
        ...
        # 迭代计算SimRank值
        x = random.choice(list(neigh_u))
        y = random.choice(list(neigh_v))
        # 如果选择的两个节点相同，则跳过当前迭代
        # 计算SimRank值
        sim += C / (len_u * len_v) * self.simRank(x, y,C,depth-1,cache) #C默认为0.7 可以不写
        # 缓存结果
        cache[(u, v)] = sim
