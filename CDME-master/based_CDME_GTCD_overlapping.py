...
class overlap_game:
    ...
    #收益函数1  U(S_(-i),s_i )=∑_(i,j)〖1/2(〖Intimacy〗_(j→i)/(|s_i|)+〖Intimacy〗_(i→j)/(|s_j|))〗|s_i⋂s_j|
    ...
    def utility_function(self,u,c): # 节点u在社区c中的收益
       ...
       for node in self.G.nodes():
           if c in self.node_community[node]: #如果这个节点的策略包含c 就计算u和这个节点的亲密度
               ...
               # intersection = self.node_community[u] & self.node_community[node]
               # s1 = len(self.node_community[u])
               # s2 = len(self.node_community[node])
               # paysoff_u = paysoff_u + 1/2 * (list[0] / s1 + list[1] / s2) * len(intersection)
               paysoff_u = paysoff_u + 1/2 * (list[0] + list[1] )
       return  paysoff_u
    '''
    #收益函数2 明明看起来更复杂但这个收益函数比前面的要差
    # 参考2019年 cd based game theory 的效用函数设计
    #gain = 1/2m ∑(i,j) (Aijδ(i,j) - Intimacy didj/2m * |si ∩ sj|  δ(i,j)表示i.j是否有共同标签
    #loss = 1/2m (|si| - 1)
    def utility_function(self,u,c): # 节点u在社区c中的收益
        gain_function = 0
        for neigh in self.G.neighbors(u): #Aij = 1
            ...
            deerta = 0
            # δ(i,j)表示i.j是否有共同标签
            intersection = len(community_neigh & community_node)
            if intersection == 0:
                deerta = 0
            else:
                deerta = 1
            list = self.simintimacy(u,neigh)
            aij = 0
            if self.G.has_edge(u,neigh):
                aij = 1
            gain_function = gain_function + 1/2 * self.edge_count * (aij * deerta - (1/2 * self.edge_count * (list[0] + list[1]) * self.deg[u] * self.deg[neigh]) * intersection)

        loss_function = 1/ 2 * self.edge_count * (len(self.node_community[u]) - 1)
    ...
    def store_graphlist(self, fname):
        ...
        #修改为重叠就不能简单的用单个节点了
        #给每个节点设置社区为set(self.G.nodes()) 初始化为包含自己编号的集合
        self.node_community = {node: {node} for node in self.G.nodes()} #直接{}起来就是一个集合
        # self.node_community = dict(zip(self.G.nodes(), list(self.G.nodes())))
        ...
        for tup in sorted(self.G.degree, key=lambda x: x[1], reverse=True):  # x[1]就是度 按照度降序排列  但这个遍历输出的是元组 不是node
            ...
            if deg_node == 1:  # 如果节点的度为1，那么它的社区就是它唯一的那个邻居编号
                self.node_community[node].remove(node) # 删掉自己的初始社区
                self.node_community[node].add(list(self.G.neighbors(node))[0]) # 改为邻居社区
            else:  # 其他情况则遍历邻居
                ...
                if flag is False:  # 如果遍历完flag为false
                    ...
                    # 让当前节点加入根据亲密度选中最亲密节点selected的社区。
                    self.node_community[node].remove(node)  # 删掉自己的初始社区
                    self.node_community[node].add(selected)  # 改为邻居社区
    ...
    def community_detection_game(self, outdirpath, fname,datasetType=1):
        ...
        while itern < maxit: #开始博弈，实验maxit次
            ...
            while isChange is True: #如果有改变就继续博弈
                ...
                # TODO 尝试离开当前社区,离开哪个社区？离开当前所有社区，还是离开其中一个？
                # 应该是尝试离开策略中的每一个社区计算效用值
                leave = self.leave_community(node)
                # 开始比较三个动作之后效用函数的大小
                if join[1] > leave[1]:
                    self.node_community[node].add(join[0])  # 加入这个邻居社区  #TODO 原来会越来越好，原来玄机在这里，后面的博弈都是在前面的基础上，前面已经让这些节点加入了大概率会更好的社区 所以后面的博弈效果都很好
                    self.utility_nodes[node] = join[1] #更新效用值
                    for key in self.utility_list.keys(): #只要有一个节点发生改变，就要全部重新设为True进行博弈
                        self.utility_list[key] = True
                    print("join")
                elif leave[1] > join[1]:
                    self.node_community[node].remove(leave[0]) #离开那个使得效用值增加最大的社区
                    self.utility_nodes[node] = leave[1] #更新效用值
                    for key in self.utility_list.keys(): #只要有一个节点发生改变，就要全部重新设为True进行博弈
                        self.utility_list[key] = True
                    print("leave")
        ...
        community_ids = set()
        for node in self.G.nodes():
            for community in self.node_community[node]:
                community_ids.add(community)
        # 记录每个社区中有哪些成员
        for ids in community_ids:
            for node in self.G.nodes():
                if ids in self.node_community[node]:
                    self.graph_result[ids].append(node)  #记录社区成员
    ...
    def leave_community(self, node):
        ...
        for c in node_community:
            #计算离开c的收益
            utility_leave = self.utility_function(node, node_community - {c})
            if utility_leave > utility_now and utility_leave > max_utility:
                max_utility = utility_leave
                best_comm = c
        # 返回[c,max_utility] 要离开的社区和能得到的最大收益max_utility
    ...
    def join_neigh_community(self, node):
        ...
        for neig in neiglist:
            ...
            for comm in C:
                communities_neigh.add(comm)  # 将邻居社区添加到邻居社区集合中，用无序来保证随机访问。
        ...
        for c in communities_neigh:
            if c in node_community:  # 如果节点node已经选择了社区c就没必要算了(本来就在一起，还加入什么),算下一个相邻的c
                continue
            # 如果和当前社区不一样
    ...
    def Lables_to_Partition(self,LA):
        ...
        #set不能再套set，所以多了一步用tuple取出LA中的元素
        community_number = list(set(tuple(l) for l in LA))
        community_res = []
        for comm in community_number:
            community_res.append(set(comm))
        print(community_res)
        #遍历节点标签，看看有没有包含community的
        for community in community_res:
            for index in range(0,len(LA)): #左闭右开
                if community in LA[index]:
