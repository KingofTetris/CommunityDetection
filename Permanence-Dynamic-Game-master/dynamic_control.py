...
def dynamic_control(dirpath,outdirpath,initial = "resume",select = "random",penalty = 0.5):
    ...
    for infile in os.listdir(dirpath):  
        ...
        max_community_size.append(max(len(members) for members in cur_graph.community_map_to_node.values()))
        average_commmunity_size.append(sum(len(members) for members in cur_graph.community_map_to_node.values())/float(len(cur_graph.community_map_to_node)))
...
if __name__ == '__main__':
    ...
    # python3把raw_input舍弃了
    dirpath = input("Please enter the path of a folder under which the snapshots will be analyzed:\n")
    "Set a output folder"
