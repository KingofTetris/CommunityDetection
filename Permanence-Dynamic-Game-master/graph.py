...
class graph:
    ...
    def store_graphlist(self):
        ...
        for line in current_file:
            if line[0] != "#" and len(line)>1:
                ...
                map没有索引，修改为list
                '''
                linepair = list(linepair) #修改成list
        ...
        # self.edge_count = sum(len(i) for i in self.graphlist.values())/2
        self.edge_count = sum(len(i) for i in self.graphlist.values())/2
    ...
    def permanence_modularity_community(self,last_graph=None,initial="restart",select="random",penalty=0.5):
        ...
        if initial == "restart" or last_graph == None:
            ...
            '''
            # self.node_map_to_community = {node : {node} for node in self.graphlist.keys()}
            self.node_map_to_community = {node : {node} for node in self.graphlist.keys()}
            # self.community_map_to_node = {node : {node} for node in self.graphlist.keys()}
            self.community_map_to_node = {node : {node} for node in self.graphlist.keys()}
            ...
            disequilibrium_node_list = {node : len(neighbors) for node , neighbors in self.graphlist.items()}
            ...
            for node,communities in last_graph.node_map_to_community.items():
                # if node in self.graphlist.keys():
                if node in self.graphlist.keys():
            ...
            # for node in self.graphlist.keys():
            for node in self.graphlist.keys():
                # if node not in self.node_map_to_community.keys():
                if node not in self.node_map_to_community.keys():
                    singleton_community = node
                    # while singleton_community in self.community_map_to_node.keys():
                    while singleton_community in self.community_map_to_node.keys():
            ...
            for each_node,neighbors in self.graphlist.items():
    ...
    def initial_singleton_community(self,selected_node):
        ...
        # if selected_node in self.node_map_to_community.keys():
        if selected_node in self.node_map_to_community.keys():
    ...
    def join_a_community(self,selected_node):
        ...
        for each_node in self.graphlist[selected_node]:
            utility_neighbors_now += self.utility_list[each_node]
        # max_community_number = max(join_community_dict.values())
        max_community_number = max(join_community_dict.values())
        ...
        join_community_set = {x for x,y in join_community_dict.items() if y==max_community_number}
    ...
    def switch_a_community(self,selected_node):
        ...
        if len(community_number_dict) == 0:
            return -100,-200,{},-100
        e_max = max(community_number_dict.values())
        ...
        if e_max == 1:
            ...
            switch_community_set.update({community for (community,community_number) in community_number_dict.items() if community_number == e_max})
