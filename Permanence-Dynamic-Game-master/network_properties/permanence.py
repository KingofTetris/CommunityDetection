...
def permanence_node(g,node_v):
    ...
    if not isinstance(g,graph.graph):
        ...
        return None
    """
    if node_v not in g.graphlist.keys():
...
def permanence(g):
    ...
    '''
    permanence_sum = 0
    for node_v in g.graphlist.keys():
        permanence_sum += permanence_node(g,node_v)
    return permanence_sum / sum(len(g.node_map_to_community[neighboring_node]) for neighboring_node in g.graphlist.keys())
